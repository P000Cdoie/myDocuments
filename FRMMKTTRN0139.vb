    ...
    Private Sub FN_Spread_Settings()
        ...
        With SSPurOrd
            ...
            Else
                ...
                If UCase(Trim(gstrUNITID)) = "MST" Or UCase(Trim(gstrUNITID)) = "MSB" Then
...
ErrHandler:
    ...
    Private Sub cmdCustHelp_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdCustHelp.Click
        ...
        If SchUpdFlag = True Then
            strHelp = ctlHelp.ShowList(gstrCONNECTIONSERVER, gstrDSNName, gstrDatabaseName, "Select customer_Code,cust_name from customer_mst where UNIT_CODE='" & gstrUNITID & "' and SCH_UPLOAD_CODE ='SOUPLOAD' and ((isnull(deactive_flag,0) <> 1) OR (cast(getdate() AS date)<= deactive_date))", "Help", 1)
        Else
            strHelp = ctlHelp.ShowList(gstrCONNECTIONSERVER, gstrDSNName, gstrDatabaseName, "Select customer_Code,cust_name from customer_mst where UNIT_CODE='" & gstrUNITID & "' and ((isnull(deactive_flag,0) <> 1) OR (cast(getdate() AS date)<= deactive_date))", "Help", 1)
        End If
    ...
    Function Save_DailyMktSchedule() As Boolean
        ...
        With SSPurOrd
            For Row = 1 To .MaxRows
                ...
                rsPrevQty.Open("select isnull(sum(isnull(d.order_qty,0)),0) as PrevQty from cust_ord_hdr h, cust_ord_dtl d " &
                    " where d.cust_ref = h.cust_ref and " &
                    " h.amendment_no= d.amendment_no and" &
                    " d.UNIT_CODE = h.UNIT_CODE and " &
                    " h.account_code= d.account_code and" &
                    " d.item_code = '" & varItemCode & "' and " &
                    " h.account_code= '" & txtCustomerCode.Text & "'" &
                    " and month(h.order_date) = " & Month(getDateForDB(dtpSOdate.Value)) & " " &
...
ErrHandler:
    ...
    Private Sub Upload_SO()
        ...
        If Rsdoc.GetNoRows > 0 Then
            ...
            rsPrevDocNo.Open("select max(doc_no) as doc_no from so_upld_dtl where Month(so_date) = " & Month(getDateForDB(dtpSOdate.Value)) & " " &
                ...
                While Row <= SSPurOrd.MaxRows
                    With SSPurOrd
                        ...
                        Else
                            blnopenso = 0
                        End If
                        strReturnCustRef = CheckForMultipleOpenSO(txtCustomerCode.Text, txtCustomerCode.Text, "", varPARTNO, varItemCode, blnopenso)
                    ...
                    StrSql = StrSql & " Quantity,DespatchQty,PrevQty,EDPer,ExWorks,VAT,CustSupply,ToolCost,packing_per,"
                    StrSql = StrSql & " Remarks, Doc_No, Currency_Code,"
                    ...
                    StrSql = StrSql & "'" & PrevQty & "', '" & "'," & decexworks & ","
                    StrSql = StrSql & "'" & LTrim(RTrim(varVAT)) & "', '" & Replace(varCustSupply, "'", "''") & "',"
                    StrSql = StrSql & "" & varToolCost & "," & varpackingper & ",'" & Replace(varRemarks, "'", "''") & "',"
                    StrSql = StrSql & "'" & Rsdoc.GetValue("DOC_NO") & "','" & Trim(LblCurrencyCode.Text) & "',"
                    ...
                    If blnlinelevelcustomer = True Then
                        ...
                        StrSql = StrSql & "'" & Trim(m_strDeliverySchedule) & "','" & Trim(txtconscode.Text) & "','" & mP_User & "','" & varSHOPCODE & "','" & varUOM & "','" & gstrUNITID & "','" & VARINTERNALSALESORDER & "','" & vardISCOUNTTYPE & "','" & vardISCOUNTPER & "'"
                    Else
                        StrSql = StrSql & "'" & Trim(m_strDeliverySchedule) & "','" & Trim(txtconscode.Text) & "','" & mP_User & "','" & varSHOPCODE & "','" & varUOM & "','" & gstrUNITID & "','" & Replace(varSONO, "'", "''") & "','" & vardISCOUNTTYPEDESC & "','" & vardISCOUNTPER & "'"
                    End If
                    '10624527
                    If gblnGSTUnit = True Then
...
ErrHandler:
        Call gobjError.RaiseError(Err.Number, Err.Source, Err.Description, mP_Connection)
    End Sub
    Private Sub txtAddExcise_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs)
        Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
        On Error GoTo ErrHandler
        If KeyAscii = 13 Then System.Windows.Forms.SendKeys.Send("{tab}")
        GoTo EventExitSub
...
EventExitSub:
        eventArgs.Cancel = Cancel
    End Sub
    Private Sub txtECESS_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs)
        Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
        On Error GoTo ErrHandler
        If KeyAscii = 13 Then System.Windows.Forms.SendKeys.Send("{tab}")
        GoTo EventExitSub
...
EventExitSub:
        ...
        End If
    End Sub
    Private Sub txtSECESS_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs)
        Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
        On Error GoTo ErrHandler
        If KeyAscii = 13 Then System.Windows.Forms.SendKeys.Send("{tab}")
        GoTo EventExitSub
...
EventExitSub:
    ...
    Private Function SO_Upload() As Object
        ...
        While strresult <> ""
            ...
            rsItemCode.GetResult("SELECT DISTINCT ITEM_CODE,item_desc,DRG_DESC FROM CUSTITEM_MST" &
            ...
            End If
            If blnSOUPLD_NotRemovehypen = True Then
                RSDUPLICATE.GetResult("select DISTINCT C.ACCOUNT_CODE, C.cust_drgno,COUNT(C.item_code)" &
                " countitem from custitem_mst C with (nolock) where C.active = 1 " &
                " and CUST_DRGNO = '" & Obj_EX.Range("$E$" & Row_Ex).Value2 & "' " &
                " AND ACCOUNT_CODE = '" & txtCustomerCode.Text & "' and UNIT_CODE = '" & gstrUNITID & "' GROUP BY ACCOUNT_CODE, CUST_DRGNO")

            Else
                RSDUPLICATE.GetResult("select DISTINCT C.ACCOUNT_CODE, C.cust_drgno,COUNT(C.item_code)" &
                " countitem from custitem_mst C with (nolock) where C.active = 1 " &
                " and CUST_DRGNO = '" & Replace(Obj_EX.Range("$E$" & Row_Ex).Value2, "-", "") & "' " &
                " AND ACCOUNT_CODE = '" & txtCustomerCode.Text & "' and UNIT_CODE = '" & gstrUNITID & "' GROUP BY ACCOUNT_CODE, CUST_DRGNO")
            End If
            If RSDUPLICATE.GetNoRows > 0 Then
                ...
                Else
                    '10530629 
                    If Not IsRecordExists("select top 1 1 from item_mst where UNIT_CODE = '" & gstrUNITID & "'" &
                            " and Item_Code = '" & rsItemCode.GetValue("ITEM_CODE") & "'" &
            ...
            With SSPurOrd
                ...
                ElseIf UCase(Trim(gstrUNITID)) = "MST" Or UCase(Trim(gstrUNITID)) = "MSB" Or UCase(Trim(gstrUNITID)) = "MAR" Or UCase(Trim(gstrUNITID)) = "MS1" Then
                    .SetText(ENUM_Grid.ExWorks, .MaxRows, Obj_EX.Range("$J$" & Row_Ex).Value2.ToString())
                ...
                If Not (UCase(Trim(gstrUNITID)) = "MPU" Or UCase(Trim(gstrUNITID)) = "SMC" Or UCase(Trim(gstrUNITID)) = "SMP" Or UCase(Trim(gstrUNITID)) = "MST" Or UCase(Trim(gstrUNITID)) = "MSB") Or UCase(Trim(gstrUNITID)) = "MNK" Or UCase(Trim(gstrUNITID)) = "MC2" Then
                ...
                'gst changes
                If UCase(Trim(gstrUNITID)) = "MPU" Or UCase(Trim(gstrUNITID)) = "SMC" Or UCase(Trim(gstrUNITID)) = "SMP" Or UCase(Trim(gstrUNITID)) = "MRG" Or UCase(Trim(gstrUNITID)) = "MNK" Or UCase(Trim(gstrUNITID)) = "MC2" Then
...
ErrHandler:
    ...
    Private Sub frmMain_Enter(sender As Object, e As EventArgs) Handles frmMain.Enter

    End Sub

    Private Sub cmdconshelp_Click(sender As Object, e As EventArgs) Handles cmdconshelp.Click
        On Error GoTo ErrHandler
        Dim strHelp() As String
        With ctlHelp
            .CreateDSN(gstrCONNECTIONSERVER, gstrCONNECTIONDESCRIPTION, gstrCONNECTIONDSN, gstrCONNECTIONDATABASE)
        ...
        If UBound(strHelp) <> -1 Then
            If strHelp(0) <> "0" Then
                If strHelp(0) <> "" Then
                    Me.txtconscode.Text = strHelp(0)
                    Me.lblconsigneename.Text = strHelp(1)
                    Me.txtconscode.Focus()
                End If
            Else
                Me.txtconscode.Text = ""
