...
Public Class frmMKTTRN0082
    ...
    Private Sub cmdDocCode_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDocCode.Click
        ...
        Try
            StrDocHelp = ShowList(0, Len(txtDocCode.Text), txtDocCode.Text, "Doc_No", "" & DateColumnNameInShowList("Trans_date") & " as Trans_Date", "WIP_FG_PICKLIST_INV_HDR", " AND TRANS_DATE= CONVERT(VARCHAR(12),GETDATE(),106) ", "WIP INVOICE PICKLIST SERIES ")
    ...
    Private Function InsertIntoPickListTable() As Boolean
        ...
        Try
            ...
            strQry = "SELECT ISNULL(CURRENT_NO,0)+1 AS CURRENT_NO,FIN_START_DATE,FIN_END_DATE FROM DOCUMENTTYPE_MST WHERE DOC_TYPE=9990 AND '" & VB6.Format(lblDisplayDate.Text, "DD MMM YYYY") & "'  BETWEEN FIN_START_DATE AND FIN_END_DATE AND UNIT_CODE = '" & gstrUNITID & "'"
            ...
            strQry = "INSERT INTO WIP_FG_PICKLIST_INV_HDR(UNIT_CODE,DOC_TYPE,DOC_NO,INVOICE_TYPE,CUSTOMER_CODE,TRANS_DATE,TRANS_TIME,ENT_DT,ENT_USERID,UPD_DT,UPD_USERID )"
            strQry += " VALUES ('" & gstrUNITID & "',9990 " & ",'" & GenerateDocumentno & "','" & strinvoicetype & "','" & txtCustomerCode.Text.Trim & "',"
            ...
            With SpItemDetails
                For intLoopCounter = 1 To .MaxRows
                    ...
                    If CBool(.Value) Then
                        ...
                        strQry = "INSERT INTO WIP_FG_PICKLIST_INV_DTL(UNIT_CODE,DOC_TYPE,DOC_NO,CUST_ITEM_CODE,CUST_ITEM_DESC,ITEM_CODE,QUANTITY,SO_NO,AMENDMENT_NO,ENT_DT,ENT_USERID)"
                        strQry += "VALUES ('" & gstrUNITID & "',9990 " & ",'" & GenerateDocumentno & "','" & StrPartCode & "','" & StrDrgdesc & "','" & stritemcode & "'," & dblquantity
            ...
            strQry = "UPDATE DOCUMENTTYPE_MST SET CURRENT_NO=" & Val(CStr(Documentno)) & " WHERE UNIT_CODE='" & gstrUNITID & "'AND DOC_TYPE=9990 AND  GETDATE() BETWEEN FIN_START_DATE AND FIN_END_DATE  "
            sqlCmd.CommandType = CommandType.Text
            sqlCmd.CommandText = strQry
            sqlCmd.ExecuteNonQuery()
        ...
        Try
            ...
            Select Case CmdBUTTON.Mode
                Case UCActXCtl.clsDeclares.ModeEnum.MODE_ADD
                    ...
                    strCustHelp = ShowList(0, Len(txtunitcode.Text), , "CUSTOMER_CODE", "CUST_NAME", "CUSTOMER_MST", " AND UNIT_CODE ='" & gstrUNITID & "' AND CUST_WIP_FG_FUNCTIONALITY =1 ", "CUSTOMER HELP", , , , , )
                    ...
                    Else
                        ...
                        lblCustomerdesc.Text = GetQryOutput("SELECT CUST_NAME FROM CUSTOMER_MST WHERE UNIT_CODE='" & gstrUNITID & "' AND CUSTOMER_CODE='" & strCustHelp & "' AND CUST_WIP_FG_FUNCTIONALITY =1")
                    End If
            End Select
    ...
    Private Sub txtCustomerCode_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtCustomerCode.Validating
        ...
        Try
            ...
            If CmdBUTTON.Mode = UCActXCtl.clsDeclares.ModeEnum.MODE_ADD Then
                ...
                strQry += " WHERE CUSTOMER_CODE= '" & Me.txtCustomerCode.Text.Trim & "' and unit_code= '" & gstrUNITID & "' AND CUST_WIP_FG_FUNCTIONALITY =1 "
            End If
            Call GetData(strQry, dtCustomer)
            If dtCustomer.Rows.Count = 0 Then
                MsgBox("Invalid Customer Code for WIP ", MsgBoxStyle.Information, ResolveResString(100))
    ...
    Public Function FILLSODETAILS() As Boolean
        ...
        Try
            ...
            With sqlCmd
                ...
                .CommandText = "USP_WIPFG_PICKLIST_GENERATE_DATA"
            ...
            With SpItemDetails
                For lngloop = 0 To ods.Tables(0).Rows.Count - 1
                    ...
                    StrCustDrgdesc = CStr(ods.Tables(0).Rows(lngloop).Item("CUST_DRGNO"))
                    ...
                    strQry = "SELECT DBO.UDF_WIPFG_CURRENTMONTHINVOICE('" & gstrUNITID & "','" & txtCustomerCode.Text.Trim & "','" & StrCustDrgno & "','" & Stritemcode & "')"
                    dblinvoiceQty = SqlConnectionclass.ExecuteScalar(strQry).ToString()

                    strQry = "SELECT DBO.UDF_WIPFG_CURRENTDATE_PENDING_PICKLIST('" & gstrUNITID & "','" & txtCustomerCode.Text.Trim & "','" & StrCustDrgno & "','" & Stritemcode & "')"
                    ...
                    End If
                Next
                .Row = 1 : .Col = EnmGridCol.ItemCode : .Row2 = .MaxRows : .Col2 = EnmGridCol.ScheduleQuantity : .BlockMode = True : .Lock = True : .BlockMode = False
    ...
    Private Sub AddColumnsInSpread()
        ...
        Try
            With SpItemDetails
                ...
                .Col = EnmGridCol.Mark : .Text = "Mark" : .set_ColWidth(EnmGridCol.Mark, 300)
                ...
                .Col = EnmGridCol.ScheduleQuantity : .Text = "Pending Schedule" : .set_ColWidth(EnmGridCol.ScheduleQuantity, 1000)
                .Col = EnmGridCol.Pending_WIPPick_Currentdate : .Text = "Pending WIP QTY for Current Date " : .set_ColWidth(EnmGridCol.Pending_WIPPick_Currentdate, 1200)
                .Col = EnmGridCol.Quantity_to_be_Invoice : .Text = "Quantity to be Invoice" : .set_ColWidth(EnmGridCol.Quantity_to_be_Invoice, 1600)
                .Col = EnmGridCol.ItemCode : .Col2 = EnmGridCol.ScheduleQuantity : .BlockMode = True : .Lock = True : .BlockMode = False
            ...
            Select Case e.Button
                ...
                Case UCActXCtl.clsDeclares.ButtonEnum.BUTTON_DELETE
                    If Len(txtDocCode.Text) > 0 And SpItemDetails.MaxRows > 0 Then
                        mblndatadeletion_req = DataExist("SELECT TOP 1 1 FROM WIP_FG_PICKLIST_INV_DTL  WHERE  UNIT_CODE='" & gstrUNITID & "' AND DOC_NO ='" & txtDocCode.Text.Trim & "' AND TMP_INVOICENO IS NOT  NULL ")
                        If mblndatadeletion_req = False Then
                            ...
                            End If
                        Else
                            MsgBox("Pick List is Scanned , Not possible to delete ", MsgBoxStyle.Information, ResolveResString(100))
                        End If
                    Else
                    End If
            End Select
                ...
                Case UCActXCtl.clsDeclares.ModeEnum.MODE_VIEW
                    ...
                    If dtCust.Rows.Count > 0 Then
                        ...
                        With SpItemDetails
                            ...
                            .Col = EnmGridCol.Salesorder : .Row = .MaxRows : .Col2 = EnmGridCol.Pending_WIPPick_Currentdate : .Row2 = .MaxRows : .BlockMode = True : .ColHidden = True : .BlockMode = False
                            .Enabled = False
                        End With
    ...
    Private Function DeleteRecord() As Boolean
        ...
        Try
            ...
            strQry = "DELETE FROM WIP_FG_PICKLIST_INV_DTL WHERE UNIT_CODE='" + gstrUNITID + "' AND DOC_NO='" & txtDocCode.Text.Trim & "'"
            sqlCmd.CommandType = CommandType.Text
            sqlCmd.CommandText = strQry
            sqlCmd.ExecuteNonQuery()

            strQry = "DELETE FROM WIP_FG_PICKLIST_INV_HDR WHERE UNIT_CODE='" + gstrUNITID + "' AND DOC_NO='" & txtDocCode.Text.Trim & "'"
            sqlCmd.CommandType = CommandType.Text
            sqlCmd.CommandText = strQry
            sqlCmd.ExecuteNonQuery()
    ...
    Private Sub cmdSo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            StrSOHelp = ShowList(0, Len(txtSalesOrder.Text), txtSalesOrder.Text, "Cust_ref", "" & DateColumnNameInShowList("Amendment_No") & " as Amendment_No", "VW_ALL_ACTIVESALESORDER", "AND ACCOUNT_CODE='" & Me.txtCustomerCode.Text.Trim & "'  ", "SALES ORDER LIST")
            If StrSOHelp = "-1" Then
    ...
    Private Sub txtSalesOrder_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtSalesOrder.Validating
        ...
        Try
            Call ChangeMousePointer(eMPowerFunctions.ObjectsEnum.obj_Screen, , System.Windows.Forms.Cursors.WaitCursor)
            If CmdBUTTON.Mode = UCActXCtl.clsDeclares.ModeEnum.MODE_ADD Then
                strQry = "SELECT CUST_REF FROM VW_ALL_ACTIVESALESORDER  "
            ...
            If dtsalesorder.Rows.Count = 0 Then
                MsgBox("No Sales Order exists for WIP ITEMS ", MsgBoxStyle.Information, ResolveResString(100))
                Call ChangeMousePointer(ObjectsEnum.obj_Screen, , Cursors.Default)
                txtSalesOrder.Text = String.Empty
    ...
    Private Sub cmdsono_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdsono.Click
        ...
        Try
            ...
            StrSOHelp = "SELECT CUST_REF, AMENDMENT_NO FROM VW_ALL_ACTIVESALESORDER WHERE UNIT_CODE='" & gstrUNITID & "' AND ACCOUNT_CODE='" & Me.txtCustomerCode.Text.Trim & "'  "
