...
Friend Class frmMKTTRN0071a
    ...
    '============================================================================================
    '10624398 
    Public Strsalesorder As String
    '10624398 
        ...
        With SpItems
            For intSubItem = 1 To .MaxRows
                ...
                If CBool(.Value) = True Then
                    ...
                    strSchDate = Convert.ToDateTime(Trim(.Text))
                    ...
                    If gblnGSTUnit Then
                        ...
                        End If
                        sqlCmd.Dispose()
                        mstrItemText = mstrItemText & StrItemCode & "|" & strDrawingNo & "|" & strDescription & "|" & dblQuantity & "|" & dbldespatchQty & "|" & strCustRef & "|" & StrAmendmentNo & "|" & getDateForDB(strSchDate) & "|" & strSchTime & "|" & strAccountCode & "|" & strSrvDINo & "|" & intlineno & "|" & hsnsacType & "|" & hsnsacCode & "|" & cgstType & "|" & cgstPercent & "|" & sgstType & "|" & sgstPercent & "|" & igstType & "|" & igstPercent & "|" & utgstType & "|" & utgstPercent & "|" & CompensationCessType & "|" & CompensationCessPercent & "^"
                    Else
                        mstrItemText = mstrItemText & StrItemCode & "|" & strDrawingNo & "|" & strDescription & "|" & dblQuantity & "|" & dbldespatchQty & "|" & strCustRef & "|" & StrAmendmentNo & "|" & getDateForDB(strSchDate) & "|" & strSchTime & "|" & strAccountCode & "|" & strSrvDINo & "|" & intlineno & "^"
                    End If
                    '101188073 End
...
ErrHandler:
    ...
    Sub AddColumnsInSpread()
        With SpItems
            ...
            .Col = GridHeader.Line_No : .Text = "Serial No " : .set_ColWidth(GridHeader.Line_No, 12)
            '101188073 Start
            If gblnGSTUnit Then
    ...
    Public Function SelectDatafromItem_Mst(Optional ByRef pstrItem As String = "", Optional ByRef intAlreadyItem As Integer = 0) As Object
        ...
        strItembal = "select Distinct line_no, PDSNO, m.item_code, m.cust_drgNo, m.Description, m.Quantity, m.Cust_Ref, m.amendment_no,( CONVERT( VARCHAR(11),PDSDATE,106)+' '+  CONVERT(VARCHAR(6),SCH_TIME))  AS 'pdsdate',"
        ...
        End If
        '101188073 End
        strItembal = strItembal & " from vw_ToyotaPDS_Help m (NOLOCK) "
        ...
        Else
            strItembal = strItembal & " where m.Unit_code = '" & gstrUNITID & "' and  ( M.PDSNO ='" & Me.TxtPDSNo.Text & "' AND M.CUST_REF='" & Me.TxtSONo.Text & " '  AND  M.PDSDATE >= '" & getDateForDB(mstartingdate) & "'  and m.quantity > ((select isnull(sum(b.sales_quantity),0) from salesChallan_dtl a inner join sales_dtl b on a.unit_code = b.unit_code and  a.location_code = b.location_code and a.doc_no=b.doc_no where a.unit_code = '" & gstrUNITID & "' and  m.pdsno = b.srvdino and a.cancel_flag <> 1  and b.item_code=m.item_code and b.cust_item_code=m.cust_drgno and a.account_code=m.account_code))"
        End If

        strItembal = strItembal & ") order by line_no "
        ...
        If intRecordCount > 0 Then '          'if record found
            ...
            For INTCOUNT = 1 To intRecordCount
                With SpItems
                    ...
                    .Col = GridHeader.SchDate
                    .Text = Convert.ToDateTime(rsItembal.GetValue("PDSDATE")).ToString(gstrDateFormat) & " " & Convert.ToDateTime(rsItembal.GetValue("PDSDATE")).ToString("HH:mm") : .Lock = True
                    .Col = GridHeader.AccountCode
                    .Text = rsItembal.GetValue("Account_Code") : .Lock = True
                    .Col = GridHeader.Line_No
                    .Text = rsItembal.GetValue("line_no") : .Lock = True
                    '101188073 Start
                    If gblnGSTUnit Then
                        .Col = GridHeader.IS_HSN_SAC
...
ErrHandler:
    ...
    Function ValidateData() As Boolean
                ...
                If CBool(.value) = True Then
                    ...
                    Else
                        ...
                        If UCase(StrItemCode) = UCase(strOtherItemCode) Then
                            strMessage = "selected same item code more than once " & vbCrLf
                            strMessage = strMessage & "Item code :->" & strOtherItemCode & "  " & vbCrLf
                            MsgBox(strMessage, MsgBoxStyle.Information, "eMPro")
                            ValidateData = False
                            Exit Function
                        End If
        ...
        End With
    End Function
    Public Function Find_Value(ByRef strField As String) As Object
        On Error GoTo ErrHandler
...
ErrHandler:
    ...
    Private Sub CmdPDSNo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdPDSNo.Click
        ...
        If Len(Me.TxtPDSNo.Text) = 0 Then
            ...
            strHelp = ShowList(1, (TxtPDSNo.MaxLength), "", "PDSNo ", DateColumnNameInShowList("PDSDATE", True) & " as PDSDATE", "vw_ToyotaPDS_Help M", " AND  M.PDSDATE >=  '" & getDateForDB(mstartingdate) & "'  and m.quantity > ((select isnull(sum(b.sales_quantity),0) from salesChallan_dtl a (NOLOCK) inner join sales_dtl b (NOLOCK) on a.Unit_code = b.unit_code and a.location_code = b.location_code and a.doc_no=b.doc_no where a.unit_code = '" & gstrUNITID & "' and  m.PDSNO = b.srvdino and a.cancel_flag <> 1 and m.item_code=b.item_code and m.cust_drgno=b.Cust_Item_Code and m.account_code=a.account_code  AND  INVOICE_DATE >=  '" & getDateForDB(mstartingdate) & "'  ) )")
            If strHelp = "-1" Then
            ...
            Else
                ...
                If ((CBool(Find_Value("SELECT ISNULL(MULTIPLE_SO_PDS_TOYOTA,0)as MULTIPLE_SO_PDS_TOYOTA FROM SALES_PARAMETER where unit_code = '" & gstrUNITID & "'")) = True)) Then
                    If ((CBool(Find_Value("SELECT ISNULL(PDS_TOYOTA_CUSTOMER,0)as PDS_TOYOTA_CUSTOMER  FROM CUSTOMER_MST WHERE UNIT_CODE = '" & gstrUNITID & "'AND CUSTOMER_CODE IN(SELECT TOP 1 ACCOUNT_CODE FROM vw_ToyotaPDS_Help WHERE UNIT_CODE='" & gstrUNITID & "' AND PDSNo='" & strHelp & "')")) = True)) Then
                        mblnMultipleSOPDS = True
                    End If
                End If
                If mblnMultipleSOPDS = True Then
    ...
    Private Sub CmdSONO_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSONO.Click
        ...
        If Len(Me.TxtPDSNo.Text) > 0 Then
            If Len(Me.TxtSONo.Text) = 0 Then
                '10624398 
                'strHelp = ShowList(1, (TxtPDSNo.MaxLength), "", "CUST_REF ", "AMENDMENT_NO", "vw_ToyotaPDS_Help M", "AND ( M.PDSNO = '" & Me.TxtPDSNo.Text & "' ) order by M.LINE_NO")
                If UCase(Strsalesorder) = "EXPORT INVOICE" Then
                    strHelp = ShowList(1, (TxtPDSNo.MaxLength), "", "CUST_REF ", "AMENDMENT_NO", "vw_ToyotaPDS_Help M", "AND ( M.PDSNO = '" & Me.TxtPDSNo.Text & "' ) AND M.PO_TYPE='E' ")
                Else
                    strHelp = ShowList(1, (TxtPDSNo.MaxLength), "", "CUST_REF ", "AMENDMENT_NO", "vw_ToyotaPDS_Help M", "AND ( M.PDSNO = '" & Me.TxtPDSNo.Text & "' )AND M.PO_TYPE<>'E' ")
                End If
                '10624398 END 
                ...
                End If
            Else
                'strHelp = ShowList(1, (TxtPDSNo.MaxLength), TxtPDSNo.Text, "CUST_REF", "AMENDMENT_NO", "vw_ToyotaPDS_Help M ", "AND ( M.PDSNO = '" & Me.TxtPDSNo.Text & "'  and m.cust_ref='" & Me.TxtSONo.Text & "' ) order by M.LINE_NO ")
                strHelp = ShowList(1, (TxtPDSNo.MaxLength), TxtPDSNo.Text, "CUST_REF", "AMENDMENT_NO", "vw_ToyotaPDS_Help M ", "AND ( M.PDSNO = '" & Me.TxtPDSNo.Text & "'  and m.cust_ref='" & Me.TxtSONo.Text & "' ) ")
                If strHelp = "-1" Then
                    Call ConfirmWindow(10187, eMPowerFunctions.ConfirmWindowButtonsEnum.BUTTON_OK)
                    TxtSONo.Text = ""
                    Exit Sub
