...
#Region "Form Events"
    Private Sub FRMMKTTRN0084_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            optsingleInvoice.BackColor = System.Drawing.ColorTranslator.FromOle(glngCOLOR_DISABLED)
            optMultipleInvoice.Checked = False
            optMultipleInvoice.BackColor = System.Drawing.ColorTranslator.FromOle(glngCOLOR_DISABLED)
...
#Region "Routines & Functions"
    ...
    Private Function SaveData_Multiple() As Boolean
        ...
        Dim blnSUPPINVOICE_CUSTREF As Boolean = False
            ...
            If ValidateSave() Then
                ...
                With SSinvoiceDetail
                    ...
                    For inti = 1 To .MaxRows
                        ...
                        strSqlDtl = strSqlDtl & "SGSTTAX_PER,IGSTTAX_TYPE,IGSTTAX_PER,CGST_AMT,SGST_AMT,iGST_AMT,TOTAL_AMOUNT,HSNSACCODE,TCSTAXAMOUNT,TCSTAXTYPEPER,CUST_REF,AMENDMENT_NO)"
                        ...
                        strSqlDtl = strSqlDtl & ",'" & strrefno & "','" & stramendmentno & "'"
                        strSqlDtl = strSqlDtl & ")"
                    ...
                    strSlqQuery = " INSERT INTO supplementaryData_batchwise (invoice_no,PartNo,item_code,hsnsaccode,actqty,OldRate,NewRate,DIFF_RATE,Unit_code,Account_Code,BASIC_AMOUNT,CGSTTAX_TYPE,CGSTTAX_PER,CGST_AMT,SGSTTAX_TYPE,SGSTTAX_PER,SGST_AMT,IGSTTAX_TYPE,IGSTTAX_PER,IGST_AMT,PART_DESC,INVOICE_DATE,TOTAL_AMOUNT,TCSTAXAMOUNT,CUST_REF,AMENDMENT_NO) "
                    strSlqQuery += " SELECT  " + pstrChallanNo + "+ DENSE_RANK() OVER ( ORDER BY cust_Ref,amendment_no,strPartNo),strPartNo,ITEM_CODE,hsnsaccode,SUM(stracp),strOldRate,strNewRate,DIFF_RATE,Unit_code,account_code ,sum(BASIC_AMOUNT),CGSTTAX_TYPE,"
                    strSlqQuery += " CGSTTAX_PER,sum(CGST_AMT),SGSTTAX_TYPE,SGSTTAX_PER,sum(SGST_AMT),IGSTTAX_TYPE,IGSTTAX_PER,sum(IGST_AMT),"
                    strSlqQuery += " PART_DESC,'" & getDateForDB(GetServerDate()) & "',sum(total_amount ),SUM(TCSTAXAMOUNT) ,CUST_REF,AMENDMENT_NO from TMP_supplementaryData  where unit_code='" & gstrUNITID & "' and Ipaddress='" & gstrIpaddressWinSck & "' AND account_code='" & txtCustCode.Text & "'"
                    strSlqQuery += "GROUP BY strPartNo,ITEM_CODE,hsnsaccode,strOldRate,strNewRate,DIFF_RATE,Unit_code,account_code ,CGSTTAX_TYPE, "
                    strSlqQuery += " CGSTTAX_PER,SGSTTAX_TYPE,SGSTTAX_PER,IGSTTAX_TYPE,IGSTTAX_PER,PART_DESC,cust_Ref,amendment_no"

                    SqlConnectionclass.ExecuteNonQuery(strSlqQuery)


                    'If blnSUPPINVOICE_CUSTREF = True Then
                    ...
                    strupdateSlqQuery += " AND M.cust_Ref=MB.cust_Ref AND M.amendment_no=MB.amendment_no and M.ACCOUNT_CODE=MB.ACCOUNT_CODE"
                    ...
                    strSlqQuery += " PART_DESC,'" & getDateForDB(GetServerDate()) & "',total_amount ," & " getdate(),'" & mP_User & "' ,cust_ref,amendment_no from TMP_supplementaryData  where unit_code='" & gstrUNITID & "' and Ipaddress='" & gstrIpaddressWinSck & "'"
                    SqlConnectionclass.ExecuteNonQuery(strSlqQuery)

                    strsqlHdr = "Insert into SupplementaryInv_hdr ("
                    strsqlHdr = strsqlHdr & "DRCR,SuppInvType,Unit_code,Location_Code,Account_Code,Cust_name,Doc_No,cust_ref,amendment_no,"
                    strsqlHdr = strsqlHdr & "Invoice_DateFrom, Invoice_DateTo, Invoice_Date, Bill_Flag, Cancel_flag,"
                    strsqlHdr = strsqlHdr & "Currency_Code,"
                    strsqlHdr = strsqlHdr & "Ent_dt,Ent_UserId,Upd_dt,Upd_Userid,tcstax_type,tcstax_per,item_code,cust_item_code)"
                    strsqlHdr = strsqlHdr & " select distinct '" & STRDRCR & "','" & strsuppinvtype & "','" & gstrUNITID & "','" & gstrUNITID & "',tm.Account_Code,'" & Trim(lblCustDesc.Text) & "',"
                    strsqlHdr = strsqlHdr & " invoice_no,tm.cust_Ref,tm.amendment_no,invoice_date,invoice_date,invoice_date,0,0,'INR',"
                    strsqlHdr = strsqlHdr & " '" & getDateForDB(GetServerDate()) & "','"
                    strsqlHdr = strsqlHdr & mP_User & "','" & getDateForDB(GetServerDate()) & "','" & mP_User & "','" & txtTCSTaxCode.Text & "','" & lblTCSTaxPerDes.Text & "'"
                    strsqlHdr = strsqlHdr & " ,sb.item_code,sb.PartNo "
                    ...
                    strsqlHdr = strsqlHdr & " and tm.account_code ='" & txtCustCode.Text & "'"

                    If blnSUPPINVOICE_CUSTREF = True And optsingleInvoice.Checked = True Then
                    ...
                    strSqlDtl = strSqlDtl & " sb.unit_code=tm.unit_code and sb.invoice_no=tm.invoiceno and sb.account_code=tm.account_code where tm.unit_code='" & gstrUNITID & "'"
                    strSqlDtl = strSqlDtl & " and ipaddress ='" & gstrIpaddressWinSck & "'"
                    strSqlDtl = strSqlDtl & " and sb.account_code='" & txtCustCode.Text & "'"
                    'If blnSUPPINVOICE_CUSTREF = True And optsingleInvoice.Checked = True Then
                    'strsqlHdr = strsqlHdr & " and sb.invoice_no=" & pstrcurrentChallanNo
                    'End If
                    ...
                    If blnSUPPINVOICE_CUSTREF = True Then
                        ...
                        strupdateSlqQuery += " AND SH.DOC_NO=TP.INVOICENO AND SH.account_code=TP.account_code where TP.UNIT_CODE='" & gstrUNITID & "' AND TP.IPADDRESS ='" & gstrIpaddressWinSck & "'"
                        strupdateSlqQuery += " AND TP.account_code ='" & txtCustCode.Text & "'"
                    ...
                    Else
                        ...
                        'strupdateSlqQuery += " AND SH.BILL_FLAG=0 "
                        'strupdateSlqQuery += " AND SH.ACCOUNT_CODE= '" & txtCustCode.Text & "'"
                        strupdateSlqQuery = " UPDATE SH SET SH.TOTAL_AMOUNT=XYZ.TOTAL_AMOUNT , BASIC_AMOUNT =XYZ.BASIC_AMOUNT, SH.TCSTAXAMOUNT= XYZ.TCSTAXAMOUNT FROM ("
                        strupdateSlqQuery += " SELECT SUM(BASIC_AMOUNT)BASIC_AMOUNT ,SUM(TOTAL_AMOUNT)TOTAL_AMOUNT ,SUM(TCSTAXAMOUNT)TCSTAXAMOUNT , INVOICE_NO ,UNIT_CODE FROM SUPPLEMENTARYDATA_BATCHWISE "
                        strupdateSlqQuery += " GROUP BY UNIT_CODE,INVOICE_NO )XYZ inner join SUPPLEMENTARYINV_HDR sh ON "
                        strupdateSlqQuery += "  SH.UNIT_CODE = XYZ.UNIT_CODE AND SH.DOC_NO=XYZ.INVOICE_NO"
...
ErrHandler:
    ...
    Private Sub SelectChallanNoFromSupplementatryInvHdr()
        ...
        Try
            strSql = "select TOP 1 1 FROM CUSTOMER_MST WHERE UNIT_CODE='" & gstrUNITID & "' AND CUSTOMER_CODE='" & txtCustCode.Text & "' AND SUPPINVOICE_CUSTREF=1 "

            If Convert.ToBoolean(SqlConnectionclass.ExecuteScalar(strSQL)) = True Then
                BLNSUPPINVOICE_CUSTREF = True
            Else
                BLNSUPPINVOICE_CUSTREF = False
            End If
...
#Region "Control Events"
            ...
            Select Case e.Button
                ...
                Case UCActXCtl.clsDeclares.ButtonEnum.BUTTON_SAVE
                    ...
                    If blnSUPPINVOICE_CUSTREF = True Then
                        SaveData_Multiple()
                    Else
                        MsgBox("Not specific for this customer !!")
                        Exit Sub
                    End If
    ...
    Private Sub BtnHelpHdr_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnHelpCustCode.Click
        Try
            ...
            'If optsingleInvoice.Checked = True Then
            strSQL = "Select DISTINCT Account_code,Cust_Name from VW_SUPP_INVOICE_CUSTHELP_SALESORDERWISE Where " & _
...
ErrHandler:  'The Error Handling Code Starts here
    ...
    Private Sub FN_Spread_Settings()
        ...
        With SSinvoiceDetail
            ...
            .set_ColWidth(ENUM_Gridinvoicedetails.SalesOrder, 20)
            .set_ColWidth(ENUM_Gridinvoicedetails.Amendmentno, 10)
