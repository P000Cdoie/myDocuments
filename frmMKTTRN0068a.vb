...
Friend Class frmMKTTRN0068a
    ...
    'Modified By Sanchi on 25 May 2011
    '   Modified to support MultiUnit functionality
    '=======================================================================================
    'Public mstrItemText As String
    '----------------------------------------------------------------------------------------------
...
ErrHandler:  'The Error Handling Code Starts here
    ...
    Sub AddColumnsInSpread()
        With SpItems
            ...
            .Col = GridHeader.KanbanNo : .Text = "Kenn No" : .set_ColWidth(GridHeader.KanbanNo, 10)
            ...
            .Col = GridHeader.SchDate : .Text = "Sch Date" : .set_ColWidth(GridHeader.SchDate, 13)
            ...
            .Col = GridHeader.Sequenceno : .Text = "Sequence No" : .set_ColWidth(GridHeader.Sequenceno, 10)
        End With
    End Sub
    Public Function SelectDatafromItem_Mst(Optional ByRef pstrItem As String = "", Optional ByRef intAlreadyItem As Integer = 0) As Object
        'REVISED BY VINOD FOR ISSUE ID : 10533478 - WIP CHANGES
        On Error GoTo ErrHandler
        Dim strItembal As String
        'Dim rsItembal As ClsResultSetDB
        'Dim intRecordCount As Integer 'To Hold Record Count
        'Dim intCount As Short
        'strItembal = "select Distinct KanbanNo, m.item_code, m.cust_drgNo, m.Description, m.Quantity, m.Cust_Ref, m.amendment_no,( CONVERT( VARCHAR(14),SCH_DATE,106)+' '+  CONVERT(VARCHAR(6),SCH_TIME))  AS 'Sch_Date',"
        'strItembal = strItembal & " case when Sch_Time = '23:59' then '' else Sch_Time end as sch_time, m.UNLOC, m.USLOC, m.Account_code ,m.TOOL_COST, m.sequenceno"
        'strItembal = strItembal & " from vw_Enagaredtl_Help m (NOLOCK) "
        'strItembal = strItembal & " where m.quantity > ((select isnull(sum(b.sales_quantity),0) from salesChallan_dtl a (NOLOCK) inner join sales_dtl b(NOLOCK) on a.UNIT_CODE=b.UNIT_CODE AND a.location_code = b.location_code and a.doc_no=b.doc_no where a.UNIT_CODE='" & gstrUNITID & "' AND a.invoice_date >=GETDATE() and m.kanbanNo = b.srvdino and m.item_code=b.item_code and a.bill_flag=1 and a.cancel_flag=0 AND m.UNIT_CODE=b.UNIT_CODE) ) AND m.UNIT_CODE='" & gstrUNITID & "'"
        'strItembal = strItembal & " order by sch_date desc"
        ...
        '            .Col = GridHeader.SchDate
        '            .Text = VB6.Format(rsItembal.GetValue("Sch_Date"), gstrDateFormat) : .Lock = True
        '            .Col = GridHeader.SChTime
        ...
        '            .Col = GridHeader.Sequenceno
        '            .Text = rsItembal.GetValue("sequenceno") : .Lock = True
        '        End With
        ...
        dt = SqlConnectionclass.GetDataTable("exec USP_WIPFG_KENN_INV_ITEM_HELP '" & gstrUNITID & "'")
        If dt.Rows.Count > 0 Then '          'if record found
            For Each row As DataRow In dt.Rows
                With SpItems
                    ...
                    .Col = GridHeader.Mark : .CellType = FPSpreadADO.CellTypeConstants.CellTypeCheckBox : .TypeCheckCenter = True
                    .Col = GridHeader.KanbanNo
                    .Text = row("KanbanNo").ToString : .Lock = True
                    .Col = GridHeader.ItemCode
                    .Text = row("item_code").ToString : .Lock = True
                    .Col = GridHeader.DrawingNo
                    .Text = row("cust_drgNo").ToString : .Lock = True
                    .Col = GridHeader.Description
                    .Text = row("Description").ToString : .Lock = True
                    .Col = GridHeader.Quantity
                    .Text = row("Quantity").ToString : .Lock = True
                    .Col = GridHeader.CustRef
                    .Text = row("Cust_Ref").ToString : .Lock = True
                    .Col = GridHeader.AmendmentNo
                    .Text = row("amendment_no").ToString : .Lock = True
                    .Col = GridHeader.SchDate
                    .Text = VB6.Format(row("Sch_Date"), gstrDateFormat) : .Lock = True
                    .Col = GridHeader.SChTime
                    .Text = row("Sch_Time").ToString : .Lock = True
                    .Col = GridHeader.UNLoc
                    .Text = row("UNLOC").ToString : .Lock = True
                    .Col = GridHeader.USLOC
                    .Text = row("USLOC").ToString : .Lock = True
                    .Col = GridHeader.AccountCode
                    .Text = row("Account_Code").ToString : .Lock = True
                    .Col = GridHeader.Tool_Cost
                    .Text = row("Tool_Cost").ToString : .Lock = True
                    .Col = GridHeader.Sequenceno
                    .Text = row("sequenceno").ToString : .Lock = True
                End With
            Next
                    ...
                    If CBool(.Value) = True And .Row <> eventArgs.row Then
                        ...
                        If UCase(Trim(varItemCode)) = UCase(Trim(.Text)) Then
                            ...
                            If UCase(Trim(varKanbanNo)) <> UCase(Trim(.Text)) Then
                                MsgBox("You can not select same item from different Kenn No.", MsgBoxStyle.Information, "eMPro")
                                .Col = GridHeader.Mark
...
ErrHandler:
    ...
    Sub SelectPreviousItem(ByRef pstrItem As String)
        ...
        End With
    End Sub
    Private Sub optSequenceno_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles optSequenceno.CheckedChanged
