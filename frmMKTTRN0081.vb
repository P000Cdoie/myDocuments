...
Friend Class frmMKTTRN0081
    Inherits System.Windows.Forms.Form
    Dim gblintCount As Integer = 0
    '*****************************************************************************
    'Copyright©2002             - MIND
    'Form Name (Physical Name)  - frmMKTTRN0081.frm
    'Created by                 - Saurav Kumar
    'Created Date               - 28 Nov 2013 (10476059 — eMPro- SMIIEL-MSSL EDI Linkage)
    '*****************************************************************************
    'Revised by                 - Prashant Rajpal
    'Revised Date               - 20 mar 2014 (10561739  — eMPro- DS Mismatch problem )
    '*****************************************************************************

    Private mlngFormTag As Short 'Form Tag
    Dim mstrmsgstring As String
        ...
        For Intcounter = 0 To lstStatus.Items.Count - 1
            If lstStatus.Items.Item(Intcounter).Checked Then
                ...
                If Mid(strReturn, 1, 1) = "Y" Then
                    ...
                    Dim filename As String = lstStatus.Items.Item(Intcounter).Text.Trim
                    Dim finfo As New FileInfo(filename)

                    path = Trim(Find_Value("SELECT ISNULL(MSSL_INBOUND_PATH,'') AS MSSL_INBOUND_PATH FROM SALES_PARAMETER WHERE UNIT_CODE='" & gstrUNITID & "'"))
                    File.Move(txtschdPath.Text & "\" & filename, path & "Vendor_Schedule\Loaded Files\" & filename.Substring(0, InStr(filename, ".") - 1) & "_ex" & finfo.Extension)
                Else
    ...
    Private Function UpdateMonthSchedule(ByVal strMonthlySchdFileName As String) As String
        ...
        While Len(strBuffer) > 0 'Till END of Remaining Buffer
            ...
            If Not ValidateData("CD.Cust_DrgNo", " Cust_Ord_Dtl CD,Cust_Ord_Hdr CH ", " CD.unit_Code = CH.unit_Code and CD.Account_Code = CH.Account_Code AND CD.Cust_Ref = CH.Cust_Ref AND CD.Amendment_No = CH.Amendment_No AND  CD.Account_Code = '" & stracccode & "' AND CD.Cust_DrgNo = '" & strFields(1) & "' AND CD.ACTIVE_Flag = 'A' AND CH.PO_Type = 'O' and cd.UNIT_code='" & gstrUNITID & "'") Then
                ...
                MsgBox("Customer Drawing Number " & strFields(1) & " not found." & vbCr & "Continuing with rest of the Schedule File...", MsgBoxStyle.Information, "empower")
                strException = strException & "N»Customer Drawing Number " & strFields(1) & " not found."
...
ItemCodeNotFound:
        ...
        If strException.Trim.Length > 0 Then
            UpdateMonthSchedule = strException
        Else
...
ErrHandler:
    ...
    Private Sub frmMKTTRN0027_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        ctlUploadSchedulesHDR.HeaderString = "DS Exception Uploading"
        Me.Text = "MKTTRN0081 - DS Exception Uploading"
...
ErrHandler:
    ...
    Private Function ShowNewSchedules() As Short
        ...
        txtschdPath.Text = Trim(Find_Value("SELECT ISNULL(SCHEDULE_REUPLOAD_PATH,'') AS SCHEDULE_REUPLOAD_PATH FROM SALES_PARAMETER WHERE UNIT_CODE='" & gstrUNITID & "'"))
        ...
        End If
        If gstrUNITID = "M3E" Then
            'txtschdPath.Text = "\\172.29.30.15\mssl\mate_inbound_file"
            'strbackpath = txtschdPath.Text
            'strBackpathRowWise = Split(strbackpath, "\")
            '            strbackpath = "\\" & strBackpathRowWise(1) & "\Vendor_Schedule"
        End If
        If gstrUNITID = "MAE" Then
            'txtschdPath.Text = "\\172.29.90.11\MAE_Inbound"
            'strbackpath = txtschdPath.Text
            'strBackpathRowWise = Split(strbackpath, "\")
            '            strbackpath = "\\" & strBackpathRowWise(1) & "\Vendor_Schedule"
        End If
        ...
        If Trim(strSchPath) <> "" Then
            ...
            If lstStatus.Items.Count = 0 Then 'No Schedules were found
                MsgBox("No Exception file exists.", MsgBoxStyle.Information, "empower")
                With lstStatus
                    .Enabled = False
                    .BackColor = System.Drawing.ColorTranslator.FromOle(glngCOLOR_DISABLED)
                End With
...
ErrHandler:
    ...
    Public Function DailyScheduleEntry(ByRef strSchFILE As String) As String
        ...
        Dim strSqlDeletion As New StringBuilder
        'ass2 = New Scripting.FileSystemObject
        'ass3 = New Scripting.FileSystemObject 'XML
        ...
        While Len(strBuffer) > 0
            ...
            If Not ValidateData("CD.Cust_DrgNo", " Cust_Ord_Dtl CD,Cust_Ord_Hdr CH ", " CD.unit_Code = CH.unit_Code and CD.Account_Code = CH.Account_Code AND CD.Cust_Ref = CH.Cust_Ref AND CD.Amendment_No = CH.Amendment_No AND  CD.Account_Code = '" & stracccode & "' AND CD.Cust_DrgNo = '" & strSbuItCode & "' AND CD.ACTIVE_Flag = 'A' AND PO_Type = 'O' and CD.UNIT_code='" & gstrUNITID & "'") Then
                'Customer Drwg No Not Found
                MsgBox("Customer Drawing Number " & strSbuItCode & " not found." & vbCr & "Continuing with rest of the Schedule File...", MsgBoxStyle.Information, "empower")
                strException = strException & "N»Customer Drawing Number " & strSbuItCode & " not found."
            ...
            strsql = "set dateformat 'dmy' SELECT DBO.UDF_VALIDATE_FORECAST_VS_SCHEDULE('" & gstrUNITID & "','" & stracccode & "','" & strschno & "','" & strItemCode & "') AS RESULT"
            blnresult = CBool(Find_Value(strsql))
            If blnresult = True Then

                If Len(Trim(mstrmsgstring)) = 0 Then
                    mstrmsgstring = strItemCode
                Else
                    mstrmsgstring = mstrmsgstring & "," & strItemCode
                End If

                strSqlDeletion.Append("delete from dailymktschedule_temp where unit_code ='" & gstrUNITID & "' and dsno ='" & strschno & "' and item_code ='" & strItemCode & "'")
            End If
...
ItemCodeNotFound:  'In Case Item is Not Found it is SKIPPED
        ...
        If Len(Trim(mstrmsgstring)) > 0 Then
            strErrorMessage = "Schedule qty exceeds forecast qty for item code(s) :- '" & mstrmsgstring & "'. Do you wish to upload anyway ?"
            'If MsgBox(strErrorMessage, MsgBoxStyle.YesNo + MsgBoxStyle.Information, "empower") = MsgBoxResult.No Then
            If MsgBox(strErrorMessage, MsgBoxStyle.OkOnly + MsgBoxStyle.Information, "empower") Then

                mP_Connection.BeginTrans()
                mP_Connection.Execute(strSqlDeletion.ToString())
                mP_Connection.CommitTrans()
            End If
        End If
        ...
        If blnUpdfg = True Then
            ...
            strDailymktUpdate = strDailymktUpdate & " And a.DSNo = b.DSNo and a.revisionNo =0  and a.DSNO = '" & strschno & "'"
            strDailymktUpdate = strDailymktUpdate & " and a.account_Code = '" & stracccode & "' and b.Status =1"
        ...
        mP_Connection.Execute("Update dm Set dm.Status = 0,dm.RevisionNo = dm.RevisionNo + 1 from DailyMktSchedule_temp dt ," & _
                               " Dailymktschedule dm Where dt.unit_code=dm.unit_code and dt.dsno=dm.dsno and dt.account_code=dm.account_code " & _
                               " and dt.cust_drgno=dm.cust_drgno and dt.trans_date=dm.trans_date and dm.Unit_Code='" + gstrUNITID + "' And dm.Account_Code = '" & stracccode & "' and dm.DSNO = '" & strschno & "' and dm.status=1 ", , ADODB.ExecuteOptionEnum.adExecuteNoRecords)
        '10561739  end 
        ...
        'End If
        If strException.Trim.Length > 0 Then
            DailyScheduleEntry = strException
        Else
